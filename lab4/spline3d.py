...
def findSpline(xTab, yTab, isNotaKnot = False):
    # ai x^3 + bi x^2 + ci x + di
    parts_num = len(xTab) - 1
    matrix_size = parts_num*4
    ...
    for j in range(4):
        var_matrix[0][j] = xTab[0] ** (3 - j)
        var_matrix[parts_num*2 - 1][-j-1] = xTab[-1] ** j

    for i in range(1, parts_num):
        for j in range(4):
            var_matrix[2*i - 1][j + 4*i - 4] = xTab[i] ** (3 - j)
            var_matrix[2*i][j + 4*i] = xTab[i] ** (3 - j)

    for i in range(0, parts_num-1):
        var_matrix[2*parts_num + i][4 * i] = 3 * (xTab[i+1])**2
        var_matrix[2*parts_num + i][4 * i + 1] = 2 * (xTab[i+1])
        var_matrix[2*parts_num + i][4 * i + 2] = 1
        var_matrix[2*parts_num + i][4 * i + 4] = -3 * (xTab[i+1])**2
        var_matrix[2*parts_num + i][4 * i + 5] = -2 * (xTab[i+1])
        var_matrix[2*parts_num + i][4 * i + 6] = -1

        var_matrix[3 * parts_num + i - 1][4 * i] = 6 * (xTab[i + 1])
        var_matrix[3 * parts_num + i - 1][4 * i + 1] = 2
        var_matrix[3 * parts_num + i - 1][4 * i + 4] = -6 * (xTab[i + 1])
        var_matrix[3 * parts_num + i - 1][4 * i + 5] = -2
    ...
    #Natural spline
    if not isNotaKnot:
        var_matrix[- 2][0] = 6 * xTab[0]
        var_matrix[- 2][1] = 2
        var_matrix[- 1][-4] = 6 * xTab[-1]
        var_matrix[- 1][-3] = 2
    ...
    for x in xplt:
        ...
        for i in range(num_of_pieces):
            if xTB[i] <= x < xTB[i + 1]:
                ...
                at_point = splineNotaKnot[4*i] * (x ** 3) + splineNotaKnot[4*i+1] * (x ** 2) + splineNotaKnot[4*i+2] * (x ** 1) + splineNotaKnot[4*i+3]
                ...
                at_point = splineNatural[4*i] * (x ** 3) + splineNatural[4*i + 1] * (x ** 2) + splineNatural[4*i + 2] * (x ** 1) + splineNatural[4*i + 3]
